...
class GoalTests(unittest.TestCase):
    ...
    def test_init_ok(self):
        ...
        goal = Goal("name", 0.99, datetime.datetime.strptime('05/25/2023','%m/%d/%Y').date())
        self.assertEqual(goal.name, "name") #Should initialize all parameters properly
        self.assertEqual(goal.weight, 0.99)
        self.assertEqual(goal.target_date, datetime.datetime.strptime('05/25/2023','%m/%d/%Y').date())

    def test_init_bad_name(self):
        """Assert that the class raises an error when given a bad name."""
        with self.assertRaises(TypeError):
            goal = Goal(123, 0.99, datetime.datetime.strptime('05/25/2023','%m/%d/%Y').date()) #Name should be string
            
    def test_init_bad_weight(self):
        """Assert that the class raises an error when given a bad weight."""
        with self.assertRaises(ValueError):
            goal = Goal("name", -0.01, datetime.datetime.strptime('05/25/2023','%m/%d/%Y').date()) #Weight should be a float, between or equal to 0 and 1.
        with self.assertRaises(ValueError):
            goal = Goal("name", 1.01, datetime.datetime.strptime('05/25/2023','%m/%d/%Y').date()) #Weight should be a float, between or equal to 0 and 1.   
        with self.assertRaises(TypeError):
            goal = Goal("name", "this shouldn't work", datetime.datetime.strptime('05/25/2023','%m/%d/%Y').date()) #Weight should be a float, between or equal to 0 and 1.   
