...
class ScheduleTests(unittest.TestCase):
    ...
    def test_init_ok(self):
        ...
        schedule = Schedule(int('05134'), datetime.datetime.strptime('05/28/2023').date())
        self.assertEqual(schedule.id, int('05134')) #Should initialize all parameters properly
        self.assertEqual(schedule.end_date, datetime.datetime.strptime('05/28/2023').date())

    def test_init_bad_id(self):
        """Assert that the class raises an error when given a bad id."""
        with self.assertRaises(TypeError):
            schedule = Schedule('123', datetime.datetime.strptime('05/28/2023','%m/%d/%Y').date()) #Name should be string
